    ...
    try {
      ...
      function sendLogToClient(clientId, message, isError = false) {
        ...
        if (client && client.readyState === WebSocket.OPEN) {
          const logEntry = { message: isError ? `ERROR: ${message}` : message, timestamp: Date.now() };
          
          // Store log in active process if exists
          for (const [processId, processInfo] of activeProcesses) {
            if (processInfo.clientId === clientId) {
          ...
          client.send(JSON.stringify({ type: 'backend-log', ...logEntry }));
        }
      }
