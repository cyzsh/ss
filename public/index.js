...
console.log("Persistent Client ID:", state.clientId);
    // Initialize the app
    function initApp() {
    ...
    // Check for existing process on page load (silent)
    ...
    async function handleControlAction(action) {
      ...
      let confirmText, successMessage;
      
      switch (action) {
        case 'pause':
          confirmText = 'Are you sure you want to pause the current process?';
          successMessage = 'Process paused successfully';
          break;
        case 'resume':
          confirmText = 'Are you sure you want to resume the paused process?';
          successMessage = 'Process resumed successfully';
          break;
        case 'restart':
          confirmText = 'Are you sure you want to restart the current process? This will create a new process with the original parameters.';
          successMessage = 'Process restart initiated';
          break;
        case 'stop':
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (data.success) {
            ...
            addLog(data.error || 'Failed to perform action', true);
          }
        } catch (error) {
          addLog(`Error performing action: ${error.message}`, true);
        }
      }
    }
          ...
          ${state.isSubmitting ? `
            <div class="grid grid-cols-2 gap-2 mt-4 animate-fade-in-up delay-600">
              ...
              <button
                ...
                class="control-btn bg-blue-500 hover:bg-blue-600 text-black"
              >
              ...
              <button
                ...
                class="control-btn bg-purple-500 hover:bg-purple-600 text-black"
              >
